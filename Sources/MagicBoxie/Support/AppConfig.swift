...
enum AppMode {
    /// Normal operation: discover and control the real MagicBoxie device over BLE.
    case bluetooth
    /// Dev/testing only: skip Bluetooth entirely and talk to the device's plain
    /// HTTP transport (magicboxie_device.views.web_service) directly over WiFi/
...
enum AppConfig {
    static var mode: AppMode {
        ...
        // scheme's Run action sets MAGICBOXIE_DIRECT_API=1 for convenience
        ...
        // actual MagicBoxie rather than talking to a developer's server.
        ProcessInfo.processInfo.environment["MAGICBOXIE_DIRECT_API"] == "1" ? .directAPI : .bluetooth
        #else
        .bluetooth
        #endif
    }
    ...
    static var deviceHTTPBaseURL: URL {
        if let raw = ProcessInfo.processInfo.environment["MAGICBOXIE_DEVICE_URL"],
    ...
    private static let defaultMagicBoxieWebURL = URL(string: "http://magicboxie.local")!

    /// Base URL for MagicBoxie-web, the home media server movies (and their
    ...
    ///
    /// Order of resolution:
    /// 1. `MAGICBOXIE_WEB_URL` (local dev against `go run` directly)
    ...
    static var magicBoxWebBaseURL: URL {
        if let raw = ProcessInfo.processInfo.environment["MAGICBOXIE_WEB_URL"], let url = URL(string: raw) {
            return url
        }
        ...
        return defaultMagicBoxieWebURL
    }
}
